...
export default function EventPackages() {
  ...
  const loadPackages = useCallback(async (reason = 'manual') => {
    try {
      console.log(`[EventPackages] Loading packages (${reason})...`);
  ...
  const handleAddPackage = async () => {
    ...
    try {
      ...
      Alert.alert('Success', 'Event package created');
    } catch (error) {
  ...
  const renderPackage = ({ item }: { item: Package }) => (
    <View style={[styles.packageCard, { backgroundColor: colors.surface }]}>
      ...
      <View style={[styles.typeBadge, { backgroundColor: colors.primary + '20' }]}>
        <Text style={[styles.typeText, { color: colors.primary }]}>EVENT</Text>
      </View>
      {item.duration_hours > 0 && (
        <Text style={[styles.infoText, { color: colors.textSecondary }]}>{item.duration_hours} Hours Coverage</Text>
      )}
      {item.description && (
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView showsVerticalScrollIndicator={false}>
        <ImageBackground
          source={{ uri: 'https://images.unsplash.com/photo-1511795409834-ef04bbd61622?q=80&w=2069&auto=format&fit=crop' }}
          style={styles.banner}
        >
          <LinearGradient colors={['transparent', 'rgba(0,0,0,0.8)']} style={styles.bannerGradient}>
            <Text style={styles.bannerTitle}>Event Packages</Text>
          </LinearGradient>
        </ImageBackground>
        <View style={styles.content}>
          <FlatList
            ...
            scrollEnabled={false}
            ListEmptyComponent={<Text style={{ color: colors.textTertiary, textAlign: 'center', marginTop: 40 }}>No event packages found</Text>}
          />
        </View>
      </ScrollView>
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalContainer}>
            <View style={[styles.modalContent, { backgroundColor: colors.surface }]}>
              <View style={styles.modalHeader}>
                <Text style={[styles.modalTitle, { color: colors.text }]}>New Event Package</Text>
