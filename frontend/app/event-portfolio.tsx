...
export default function EventPortfolio() {
  ...
  const loadItems = useCallback(async (reason = 'manual') => {
    setLoading(true);
    try {
      console.log(`[EventPortfolio] Loading items (${reason})...`);
      ...
      if (!result.canceled) {
        ...
        for (const asset of result.assets) {
          ...
          try {
            ...
            console.log('[Event Upload] Local file integrity:', {
            ...
            console.warn('[Event Upload] Failed to inspect local blob before upload:', probeError);
          }
          ...
          try {
            ...
            console.log('[Event Upload] Uploading image:', finalUri);
            console.log('[Event Upload] Upload URL:', uploadUrl);

            const response = await axios.post(uploadUrl, formData, {
              headers: { 'Content-Type': 'multipart/form-data' },
              timeout: 8000 // Fast fail for fallback
            });
            console.log('[Event Upload] Server response:', response.data);
            ...
            console.log('[Event Upload] Render URLs:', { serverUrl, thumbUrl });
