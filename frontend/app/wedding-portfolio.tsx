...
export default function WeddingPortfolio() {
  ...
  const loadItems = useCallback(async (reason = 'manual') => {
    setLoading(true);
    try {
      console.log(`[WeddingPortfolio] Loading items (${reason})...`);
    ...
    try {
      ...
      if (!result.canceled) {
        ...
        for (const asset of result.assets) {
          ...
          // 1. Resize to 2K (2560px) if image with high quality
          if (activeTab === 'image') {
            const manipResult = await ImageManipulator.manipulateAsync(
              asset.uri,
              [{ resize: { width: 2560 } }], // 2K standard width
          ...
          try {
            ...
            console.log('[Wedding Upload] Local file integrity:', {
            ...
            console.warn('[Wedding Upload] Failed to inspect local blob before upload:', probeError);
          }
          ...
          try {
            ...
            console.log('[Wedding Upload] Uploading image:', finalUri);
            console.log('[Wedding Upload] Upload URL:', uploadUrl);

            const response = await axios.post(uploadUrl, formData, {
              headers: { 'Content-Type': 'multipart/form-data' },
              timeout: 8000 // Fast fail for fallback
            });
            console.log('[Wedding Upload] Server response:', response.data);
            ...
            // 3. Save to Local DB using server path
            ...
            console.log('[Wedding Upload] Render URLs:', { serverUrl, thumbUrl });
