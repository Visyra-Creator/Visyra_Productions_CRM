...
export default function CommercialPackages() {
  ...
  const loadPackages = async () => {
    try {
      ...
      const result = await db.getAllAsync(
        "SELECT * FROM packages WHERE event_type = 'Commercial' ORDER BY price ASC"
      );
  ...
  const handleAddPackage = async () => {
    ...
    try {
      const db = getDatabase();
      await db.runAsync(
        'INSERT INTO packages (name, event_type, price, duration_hours, deliverables, description) VALUES (?, ?, ?, ?, ?, ?)',
        [
          formData.name,
          formData.event_type,
  ...
  const renderPackage = ({ item }: { item: Package }) => (
    <View style={[styles.packageCard, { backgroundColor: colors.surface }]}>
      ...
      <View style={[styles.typeBadge, { backgroundColor: colors.primary + '20' }]}>
        <Text style={[styles.typeText, { color: colors.primary }]}>COMMERCIAL</Text>
      </View>
      {item.description && (
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView showsVerticalScrollIndicator={false}>
        <ImageBackground
          source={{ uri: 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?q=80&w=2070&auto=format&fit=crop' }}
          style={styles.banner}
        >
          <LinearGradient colors={['transparent', 'rgba(0,0,0,0.8)']} style={styles.bannerGradient}>
            <Text style={styles.bannerTitle}>Commercial Packages</Text>
          </LinearGradient>
        </ImageBackground>
        <View style={styles.content}>
          <FlatList
            ...
            scrollEnabled={false}
            ListEmptyComponent={<Text style={{ color: colors.textTertiary, textAlign: 'center', marginTop: 40 }}>No commercial packages found</Text>}
          />
        </View>
      </ScrollView>
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalContainer}>
            <View style={[styles.modalContent, { backgroundColor: colors.surface }]}>
              <View style={styles.modalHeader}>
                <Text style={[styles.modalTitle, { color: colors.text }]}>New Commercial Package</Text>
              ...
              <ScrollView style={styles.formContent}>
                <TextInput style={[styles.input, { backgroundColor: colors.background, color: colors.text }]} placeholder="Project Name" value={formData.name} onChangeText={(text) => setFormData({ ...formData, name: text })} />
